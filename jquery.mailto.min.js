/**
 * jQuery MailTo
 * Simple jQuery plugin to hide email addresses from bot harvesters
 *
 * Depends:
 * jquery.js 1.7+
 *
 * Demo:
 * http://projects.sergiodinislopes.pt/mailto/example/
 * 
 * Github:
 * https://github.com/sergiodlopes/mailto/ 
 */
!function(t){t.fn.mailto=function(e){return e=t.extend({},{text:!1,host:window.location.hostname.replace("www.",""),account:!1,prepend:!1},e),this.each(function(){var n=t(this),o=t.extend({},e,n.data()),r=function(){if(o.account)return o.account+"@"+o.host;var t=n.text();return n.contents().filter(function(){return 1!=this.nodeType}).remove(),t.replace(" at ","@").split(" dot ").join(".")},c=r();if(o.text||(o.text=c),o.prepend?n.prepend(o.text+" "):n.append(" "+o.text),n.is("a")){var a="mailto:"+c;o.subject&&(a+="?subject="+encodeURIComponent(o.subject)),n.attr("href",a)}})}}(jQuery);